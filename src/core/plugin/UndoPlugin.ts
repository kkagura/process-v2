...
export default class UndoPlugin extends Plugin {
  //  操作栈
  list: Action[] = [];
  //  需要被合并的操作
  batch: Operation[] | null = null;
  //  当前指针
  cursor: number = 0;
  //  是否正在操作（正在操作的时候不进行记录）
  isOperating: boolean = false;
  constructor(public process: Process, public limit: number = 10) {
    super(process);
  }
  /**
   * 入栈
  ...
  push(op: Action) {
    ...
    //  如果已开启批量记录，则存入临时数组内
    if (this.batch) {
      this.batch.push(op as Operation);
      return;
    }
    ...
    console.log("[undomanager] push item:", op);
  }
  //  清空操作栈
  clear() {
    this.list = [];
